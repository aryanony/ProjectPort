...
const app = express();
const { PORT, CLIENT_ORIGIN, JWT_SECRET } = require("./config/env");
...
app.get("/api/projects/:id", authMiddleware, async (req, res) => {
  try {
    ...
    const milestones = await Milestone.find({ project_id: req.params.id }).sort({ due_date: 1 });
    const updates = await ProjectUpdate.find({ project_id: req.params.id })
      .populate('updated_by', 'full_name')
      .sort({ created_at: -1 })
      .limit(50);
      
    const payments = await Payment.find({ project_id: req.params.id }).sort({ created_at: -1 });
    const assignments = await ProjectAssignment.find({ project_id: req.params.id })
    ...
    obj.milestones = milestones.map(m => ({...m.toObject(), id: m._id}));
    obj.updates = updates.map(u => ({...u.toObject(), id: u._id, updated_by_name: u.updated_by?.full_name}));
    obj.payments = payments.map(p => ({...p.toObject(), id: p._id}));
    obj.assignments = assignments.map(a => ({...a.toObject(), id: a._id, admin_name: a.admin_id?.full_name, admin_email: a.admin_id?.email}));
...
app.get("/api/notifications", authMiddleware, async (req, res) => {
  try {
    ...
    res.json({ ok: true, notifications: notifications.map(n => ({...n.toObject(), id: n._id})) });
  } catch (err) {
...
with open("c:/Users/aRya/Desktop/ProjectPort-main/server/index.js", "w", encoding="utf-8") as f:
    f.write(code)
