...
const ClientProjectDetail = ({ user }) => {
    ...
    return (
        <>
            <SEO
                page="clientProjects"
                title={`${project.project_name} - Project Details | ProjectPort`}
                description={`Track progress, milestones, and updates for ${project.project_name}`}
                noindex={true}
                breadcrumbs={[
                    { name: 'Home', path: '/' },
                    { name: 'Projects', path: '/client/projects' },
                    { name: project.project_name, path: `/client/projects/${id}` }
                ]}
            />
